...
require('nvim-treesitter.configs').setup {
  ...
  ensure_installed = { 'c', 'cpp', 'go', 'lua', 'python', 'rust', 'tsx', 'typescript', 'vimdoc', 'vim', 'ruby', 'julia', 'javascript', 'json', 'latex', 'yaml' },
...
local make_key = function(entry)
  return string.format("%s",  entry.full_description)
end

local add_rspec_test = function(state, entry)
  state.tests[make_key(entry)] = {
    name = entry.full_description,
    line = entry.line_number,
    output = {},
  }
end

local add_rspec_output = function(state, entry)
  assert(state.tests, vim.inspect(state))
  table.insert(state.tests[make_key(entry)].output, vim.trim(entry.exception.message))
end
        ...
        on_stdout = function(_, data)
          ...
          for _, line in ipairs(data) do
            if line:find("{", 1, true) == 1 then
              local json_line = (string.sub(line, 1, line:match'^.*()}'))
              local decoded = vim.json.decode(json_line)
              print(decoded.summary.duration, " ", decoded.summary_line)
              for _, example in ipairs(decoded.examples) do
                add_rspec_test(state, example)
                mark_success(state, example)
                local test = state.tests[make_key(example)]
                if test.success and test.line then
                  ...
                  add_rspec_output(state, example)
                end
              end
            end
          end
...
vim.api.nvim_create_user_command("GoTestOnSave", function()
  attach_to_buffer(vim.api.nvim_get_current_buf(), { "rspec", "--format", "json", vim.fn.expand("%") })
end, {})
